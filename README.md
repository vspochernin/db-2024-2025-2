...
База данных "Структура организации" включает в себя пять таблиц: `Departments`, `Roles`, `Employees`, `Projects` и `Tasks`.
...
- `Employees`: Информация о сотрудниках, включая их имена, должности, руководителей, отделы и роли. Также включает ссылку на себя же для определения менеджера каждого сотрудника.
