...
**Важный момент**: в `README.md` будут приведены только условия заданий, но в скриптах-решениях будут находиться комментарии, поясняющие запросы.

## Инструкция по запуску

Учитывая, что мы имеем дело с 4-мя независимыми базами данных, было принято решение разделить их по папкам. В корне репозитория находятся 4 папки, соответственно относящиеся к перечисленным выше базам данных:
...
Внутри каждой папки также находятся по две папки:
...
1. Перейти терминале в папку `init` в папке нужной базы данных.
2. Выполнить команду `docker compose up -d` - база данных запустится в докере в фоновом режиме, а также автоматически применит скрипт инициализации.
3. Проверить работу запросов из папки `task` (подключение к БД осуществляется по `localhost:5432`, логин, пароль, название БД во всех случаях - `postgres`).
4. Завершить работу базы командой `docker compose down`.
5. Аналогичным образом проверить скрипты остальных БД.
...
База данных "Структура организации" включает в себя пять таблиц: `Tasks`, `Projects`, `Employees`, `Departments` и `Roles`.

![Схема базы данных "Структура организации"](./images/4-organization-schema.png)

- `Tasks`: Представляет задачи, назначенные сотрудникам в рамках проектов.
- `Projects`: Содержит проекты, которые ведет организация, включая информацию о датах начала и окончания, а также отделах, ответственных за проекты.
- `Employees`: Информация о сотрудниках, включая их имена, должности, руководителей, отделы и роли. Также включает ссылку на саму себя для определения менеджера каждого сотрудника.
- `Departments`: Содержит информацию о различных отделах организации, таких как отдел продаж, маркетинг, IT и др.
- `Roles`: Описывает различные роли, которые могут занимать сотрудники, например, менеджер, директор, разработчик и др.
